...
export const strapiAPI = {
  ...
  async getCarRentalBooking(id: string | number) {
    return request(
      `/car-rental-bookings/${id}${qs({ populate: "car" })}`,
      "GET"
    );
  },
  ...
  async updateCarRentalBookingStatus(id: string | number, status: string) {
    return request(`/car-rental-bookings/${id}`, "PUT", {
  ...
  async getGuestHouseBookings() {
    return request(
      `/guest-house-bookings${qs({ populate: "guest_house" })}`,
      "GET"
    );
  },
  async getGuestHouseBooking(id: string | number) {
    return request(
      `/guest-house-bookings/${id}${qs({ populate: "guest_house" })}`,
      "GET"
    );
  },
  ...
  async updateGuestHouseBookingStatus(id: string | number, status: string) {
    return request(`/guest-house-bookings/${id}`, "PUT", {
      bookingStatus: status,
    });
  },
  ...
  async upload(formData: FormData) {
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new Error(
        `Strapi upload failed: ${res.status} ${res.statusText} ${text}`
      );
    }
  ...
  async healthCheck() {
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => "");
      throw new Error(
        `Strapi health check failed: ${res.status} ${res.statusText} ${text}`
      );
    }
    return {
      ok: true,
